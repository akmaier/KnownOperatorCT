#!/usr/bin/env bash
# Helpers shared by the LME training sbatch scripts.
#
# Provides:
...
maybe_resubmit() {
    local done_marker="$1"
    local prev_rc="$2"
    local max_chain="${LME_MAX_CHAIN:-10}"
    local chain_idx="${LME_CHAIN_IDX:-1}"
    ...
    sbatch \
        ...
        --export="ALL,LME_CHAIN_IDX=${next},LME_MAX_CHAIN=${max_chain}" \
        "$0"
}
