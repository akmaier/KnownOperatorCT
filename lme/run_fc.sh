...
# Fits on any LME machine with a GPU >= 8 GB.
#
# Usage:
#   bash lme/run_fc.sh                        # GPU 0
#   CUDA_VISIBLE_DEVICES=1 bash lme/run_fc.sh # GPU 1
set -euo pipefail
...
python src/ct_train.py --config configs/ct_fc_lme.yaml --model fully_connected

echo ""
echo "=== Evaluating Fully Connected ==="
python src/ct_eval.py --config configs/ct_fc_lme.yaml --model fully_connected
